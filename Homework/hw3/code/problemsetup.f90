...
module problemsetup
  ...
  implicit none !some letters set automatically as integers, use this to avoid that
  private
...
contains
  ...
  elemental function forcing(x) result(ffunc)
    real (fp), intent(in) :: x
    real (fp) :: ffunc
    ffunc = exp(sin(x)) * (1+sin(x)-cos(x)**2) + exp(cos(3*x))*(9*sin(3*x)**2 - 9*cos(3*x)-1)
